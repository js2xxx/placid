...
fn derive_init(input: &DeriveInput, pinned: bool) -> std::result::Result<TokenStream, Error> {
...
pub fn init_pin(input: &DeriveInput) -> Result<TokenStream> {
    derive_init(input, true)
}

pub fn init(input: &DeriveInput) -> Result<TokenStream> {
    let mut tt = derive_init(input, false)?;
    tt.extend(derive_init(input, true)?);
    Ok(tt)
}
