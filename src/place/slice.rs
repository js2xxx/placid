...
impl<'a, T, S: PlaceState> PlaceRef<'a, [T], S> {
    ...
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3]);
    /// let (first, rest) = slice.split_first_own().unwrap();
    /// assert_eq!(*first, 1);
    /// assert_eq!(rest.len(), 2);
    /// ```
    #[inline]
    #[must_use]
    pub const fn split_first_own(self) -> Option<(PlaceRef<'a, T, S>, Self)> {
        if !self.is_empty() {
    ...
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3]);
    /// let (last, rest) = slice.split_last_own().unwrap();
    /// assert_eq!(*last, 3);
    /// assert_eq!(rest.len(), 2);
    /// ```
    #[inline]
    #[must_use]
    pub const fn split_last_own(self) -> Option<(PlaceRef<'a, T, S>, Self)> {
        if !self.is_empty() {
            let inner = self.inner;
            mem::forget(self);
    ...
    /// use placid::prelude::*;
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3, 4]);
    /// let (chunk, rest) = slice.split_first_chunk_own::<2>().unwrap();
    /// assert_eq!(*chunk, [1, 2]);
    /// assert_eq!(rest.len(), 2);
    /// ```
    #[inline]
    pub const fn split_first_chunk_own<const N: usize>(
        self,
    ) -> Result<(PlaceRef<'a, [T; N], S>, Self), Self> {
        if self.len() >= N {
            let inner = self.inner;
            mem::forget(self);
    ...
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3, 4]);
    /// let (chunk, rest) = slice.split_last_chunk_own::<2>().unwrap();
    /// assert_eq!(*chunk, [3, 4]);
    /// assert_eq!(rest.len(), 2);
    /// ```
    #[inline]
    pub const fn split_last_chunk_own<const N: usize>(
        self,
    ) -> Result<(PlaceRef<'a, [T; N], S>, Self), Self> {
        if self.len() >= N {
            let inner = self.inner;
    ...
    #[inline]
    #[must_use]
    pub const unsafe fn split_at_own_unchecked(self, mid: usize) -> (PlaceRef<'a, [T], S>, Self) {
        let inner = self.inner;
        mem::forget(self);
    ...
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3, 4]);
    /// let (first, second) = slice.split_at_own_checked(2).unwrap();
    /// assert_eq!(*first, [1, 2]);
    /// assert_eq!(*second, [3, 4]);
    /// ```
    #[inline]
    pub const fn split_at_own_checked(
        self,
        mid: usize,
    ) -> Result<(PlaceRef<'a, [T], S>, Self), Self> {
        if self.len() >= mid {
            // SAFETY: We just checked that `mid` is a valid index into the slice, so the
            // resulting slices are guaranteed to be valid.
            Ok(unsafe { self.split_at_own_unchecked(mid) })
        } else {
            Err(self)
        }
    }
    ...
    /// [`split_at_own_checked`](Own::split_at_own_checked).
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3, 4]);
    /// let (first, second) = slice.split_at_own(2);
    /// assert_eq!(*first, [1, 2]);
    /// assert_eq!(*second, [3, 4]);
    /// ```
    #[inline]
    #[must_use]
    pub const fn split_at_own(self, mid: usize) -> (PlaceRef<'a, [T], S>, Self) {
        assert!(mid <= self.len(), "index out of bounds");

        // SAFETY: We just checked that `mid` is a valid index into the slice, so the
        // resulting slices are guaranteed to be valid.
        unsafe { self.split_at_own_unchecked(mid) }
    }
    ...
    /// ```
    #[inline]
    pub const fn to_array<const N: usize>(self) -> Result<PlaceRef<'a, [T; N], S>, Self> {
        if self.len() == N {
            let inner = self.inner;
            mem::forget(self);
    ...
    #[inline]
    #[must_use]
    pub const unsafe fn to_chunks_unchecked<const N: usize>(self) -> PlaceRef<'a, [[T; N]], S> {
        let inner = self.inner;
        mem::forget(self);
    ...
    /// use placid::prelude::*;
    ///
    /// let slice: Own<[i32]> = own!([1, 2, 3, 4, 5]);
    ...
    #[inline]
    #[must_use]
    pub fn to_chunks<const N: usize>(self) -> (PlaceRef<'a, [[T; N]], S>, Self) {
        assert!(N != 0, "chunk size must be non-zero");

        let len_rounded_down = self.len() / N * N;
        // SAFETY: The rounded-down value is always the same or smaller than the
        // original length, and thus must be in-bounds of the slice.
        let (multiple_of_n, remainder) = unsafe { self.split_at_own_unchecked(len_rounded_down) };
...
impl<'a, T, const N: usize, S: PlaceState> PlaceRef<'a, [[T; N]], S> {
    ...
    /// use placid::prelude::*;
    ///
    /// let slice: Own<[[i32; 2]]> = own!([[1, 2], [3, 4]]);
